...
func getMySQL(host string, port int, user string, password string, db string) *databaseStruct {
	m := &databaseStruct{}
	m.driver = "mysql"
	m.dsn = user + ":" + password + "@tcp(" + host + ":" + toString(port) + ")/" + db + "?charset=utf8mb4"
...
func (e exception) except(f func(err eee)) {
	if e.Error != nil {
		defer func() {
...
func tcpConnect(host string, port int) *tcpClientSideConn {
	...
	return &tcpClientSideConn{conn: conn}
}

func (m *tcpClientSideConn) send(str string) {
	_, err := m.conn.Write([]byte(str))
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
...
func (m *tcpClientSideConn) recv(buffersize int) string {
	reply := make([]byte, buffersize)
	n, err := m.conn.Read(reply)
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
...
func (m *udpServerSideConn) recvfrom(buffersize int) (string, *net.UDPAddr) {
	p := make([]byte, buffersize)
...
func (m *udpServerSideConn) sendto(data string, address *net.UDPAddr) {
...
func getArgparser(description string) *argparseIniStruct {
	var cfg *ini.File
	var err error
	var cfgPath string
